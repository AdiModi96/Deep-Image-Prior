...
def denosie_using_noise2noise(noisy_image):
    denoised_image = network(torch.unsqueeze(torch.as_tensor(NoisyDataLoader.convert_image_to_model_input(noisy_image)), dim=0))[0]
    denoised_image = NoisyDataLoader.convert_model_output_to_image(denoised_image)
