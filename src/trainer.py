...
# --------------------------------------------------------------
hyper_paramaters = {
    # 'pretrained_model_file_path': os.path.join(paths.trained_models_folder_path, 'instance_001', 'Model_Epoch_008.pt'),
...
def train():
    ...
    train_dataset = NoisyDataLoader(dataset_type=NoisyDataLoader.TRAIN,
                                    masking_type=NoisyDataLoader.MASKING_1_MINUS_MEAN_VALUE)

    optimizer = optim.SGD(network.parameters(),
                          lr=hyper_paramaters['OPTIMIZER']['LR'],
                          momentum=hyper_paramaters['OPTIMIZER']['MOMENTUM'])
    ...
    for epoch in range(hyper_paramaters['MODEL']['NUM_EPOCHS']):
        ...
        train_dataset.shuffle()
        torch.save(network.state_dict(), os.path.join(instance_folder_path, 'Model_{}_Epoch_{}.pt'.format(hyper_paramaters['NETWORK'], str(epoch).zfill(3))))
